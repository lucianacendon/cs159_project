...
class Game:
    ...
    def testPlaceBets(self):
        ...
        while not (cur_state[1] == self.call and (cur_state[2] == 'C' or cur_state[2] == 'R')) and allowed_rounds > 0:
            ...
            print "Player %d:" % cur_player_index, cur_player.states
            if isinstance(cur_player, Agent):
                print cur_player.Q
            num_turns += 1
            if num_turns % self.player_count == 0:
                print
    ...
    def printPlayerStates(self):
        for i in range(self.player_count):
            print "Player %d:" % i, self.players[i].states


    def printPlayerQs(self):
        for i in range(self.player_count):
            if isinstance(self.players[i], Agent):
                print self.players[i].Q
